...
enum CohortStoreError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .consentRequired:
            return "Accept the current study consent before continuing."
        case .participantMismatch:
            return "That item belongs to a different study participant."
...
enum CohortStore {
    ...
    static func enroll(
        ...
        if let existing = try context.fetch(FetchDescriptor<CohortParticipant>()).first(where: {
            $0.consentVersion == consentVersion
        }) {
            return existing
        }
    ...
    static func capture(
        ...
        let enrolledParticipant = try activeParticipant(matching: participant, in: context)
        ...
        let week = StudyWeek.number(enrolledAt: enrolledParticipant.enrolledAt, at: date)
        ...
        if let assignment = assignment {
            context.insert(
                makeEvent(
                    ...
                    subjectID: item.id,
                    studyWeek: week,
                    outcome: assignment.role.rawValue
                )
            )
        }
        try save(context)
    ...
    static func assignToToday(
        ...
        context.insert(
            makeEvent(
                ...
                at: date,
                subjectID: item.id,
                studyWeek: StudyWeek.number(enrolledAt: enrolledParticipant.enrolledAt, at: date),
    ...
    static func submitCheckIn(
        ...
        if let existing = try context.fetch(FetchDescriptor<WeeklyCheckIn>()).first(where: {
            $0.participantID == enrolledParticipant.id && $0.studyWeek == week
        }) {
            return existing
        }
        ...
        let checkIn = WeeklyCheckIn(
            ...
            offerPriceCents: enrolledParticipant.offerPriceCents,
            offerCurrency: enrolledParticipant.offerCurrency
        )
    ...
    private static func activeParticipant(
        matching participant: CohortParticipant,
        in context: ModelContext
    ) throws -> CohortParticipant {
        guard let persisted = try context.fetch(FetchDescriptor<CohortParticipant>()).first(where: {
            $0.id == participant.id && $0.consentVersion == consentVersion
        }) else {
            throw CohortStoreError.consentRequired
        }
        return persisted
    }
    ...
    private static func requireOwnership(
        of item: FocusItem,
        by participant: CohortParticipant
    ) throws {
        guard item.participantID == participant.id else {
            throw CohortStoreError.participantMismatch
        }
    }
